...
% G0 not formulated as the initial condition is 0
% Formulate G
Gvec = zeros(N, 1);
rVec = ((r - 1):(N - n0 - 1))';
...
% Run closed-loop ILC and plot the response for each iteration
for ii = 1:jmax
    for jj = 1:length(rVec)
        Uj(jj) = q0 * Ujold(jj) + l0 * Ejold(jj) + kp * (Rj(jj) - Yj(jj)) + ki * sum(Ejold);
